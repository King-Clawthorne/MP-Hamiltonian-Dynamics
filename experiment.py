...
def verlet_matrix(n, duration):
    h = duration / n
    return np.array([[1.0 - h * h / 2.0, h], [-h * (1.0 - h * h / 4.0), 1.0 - h * h / 2.0]])
...
def duration_seed_map():
    keys = [(duration, denominator) for duration in DURATION_VALUES for denominator in DURATION_H_DENOMINATORS]
    ...
    for duration, denominator in keys:
        ...
        if seed in assigned:
            ...
            salt = 0
            while True:
                candidate = int(np.random.SeedSequence([73019, int(duration), denominator, salt]).generate_state(1, dtype=np.uint64)[0])
...
def main():
    ...
    for bits in PRECISION_BITS:
        for n in STOCHASTIC_STEP_COUNTS:
            ...
            stochastic_rows.append({
                ...
                "variance_trace_unbiased": float(np.var(errors, axis=0, ddof=1).sum()),
                "estimated_expected_mse": exact_bias2 + float(np.var(errors, axis=0, ddof=1).sum()),
                "rms_relative_energy_error": float(np.sqrt(np.mean(energy**2))),
            })
    ...
    for precision in ("fp32", "fp64"):
        ...
        ax.loglog([r["h"] for r in g], [r["rms_state_error"] for r in g], "o-", label=precision)
        best = min(g, key=lambda r: r["rms_state_error"])
        ax.scatter([best["h"]], [best["rms_state_error"]], s=65, marker="*", zorder=4)
    
    ax.set(xlabel="step size h", ylabel="RMS final-state error", title="Direct floating-point state rounding")
    ...
    for bits in PRECISION_BITS:
        g = [r for r in stochastic_rows if r["significand_bits"] == bits]
        ax.loglog([r["h"] for r in g], np.sqrt([r["estimated_expected_mse"] for r in g]), "o-", ms=3, label=f"s={bits}")
        best = min(g, key=lambda r: r["estimated_expected_mse"])
        ax.scatter([best["h"]], [np.sqrt(best["estimated_expected_mse"])], s=55, marker="*", zorder=4)
    ...
    for pi, bits in enumerate(PRECISION_BITS):
        group = sorted((r for r in stochastic_rows if r["significand_bits"] == bits), key=lambda r: r["steps"])
        ...
        indices = precision_boot_rng.integers(0, REPLICATES, size=(PRECISION_BOOTSTRAPS, len(group), REPLICATES))
    ...
    precision_boot_slopes = np.polyfit(np.log(fit_u), np.log(precision_boot_h[:, fit_columns]).T, 1)[0]
    ...
    for pi, bits in enumerate(PRECISION_BITS):
        ...
        precision_boot_optima.append({"significand_bits": bits, "h_ci95": [float(v) for v in interval]})
    
    u_grid = np.geomspace(fit_u.min(), fit_u.max(), 100)
    ax.set(xlabel="step size h", ylabel="RMS final-state error", title="Per-stage stochastic rounding")
    ax.grid(True, which="both", alpha=0.25)
    ...
    A_phase = T / 24.0
    fit_rows = [r for r in stochastic_rows if r["significand_bits"] >= 10 and r["significand_bits"] != DURATION_BITS and r["h"] <= 0.2]
    ...
    predicted = [(u, (0.25 * B2 * u * u * T / (A_phase * A_phase)) ** 0.2) for u in fit_u]
    fixed_b_prediction = []
    for u, h_observed, _, bits in optima:
        if bits != DURATION_BITS: continue
        h_pred = (0.25 * B2 * u * u * T / (A_phase * A_phase)) ** 0.2
    ...
    for u, h_opt, _, bits in fit_optima:
        ...
        fit_ax.annotate(f"s={bits}", (u, h_opt), xytext=(6, -4), textcoords="offset points", fontsize=9)
    ...
    for duration in DURATION_VALUES:
        ...
        for denominator in DURATION_H_DENOMINATORS:
            ...
            q, p = integrate_stochastic_rounding(n, DURATION_BITS, rng, duration, DURATION_REPLICATES)
            qe, pe = exact_state_at(np.ones(DURATION_REPLICATES), np.zeros(DURATION_REPLICATES), duration)
            errors, sample_bias2, sample_var, mse = state_error_decomposition(q, p, qe, pe)
    ...
    for ti, duration in enumerate(DURATION_VALUES):
        ...
        bias2 = np.array([r["exact_bias_squared"] for r in duration_rows if r["duration"] == duration])
        indices = boot_rng.integers(0, DURATION_REPLICATES, size=(DURATION_BOOTSTRAPS, len(denominator_array), DURATION_REPLICATES))
    ...
    duration_fit = {
        ...
        "bootstrap_resampling": "independent by duration and h, with paired state components within each trajectory",
        "optima": duration_optima,
        "rng_seed_scheme": "legacy formula retained for unique conditions and duplicate seeds are reassigned using SeedSequence([73019, duration, timestep_denominator, salt])",
        "rng_seed_collisions_reassigned": duration_seed_collisions_reassigned
    }
    
    (OUT / "duration_scaling.json").write_text(json.dumps(duration_fit, indent=2), encoding="utf-8")
    ...
    with (OUT / "crossover_fit.json").open("w", encoding="utf-8") as f:
        json.dump({
            ...
            "precision_bootstrap_resampling": "independent by significand precision and timestep, with paired state components within each trajectory",
            "precision_rng_seed_scheme": "42017 + 1000*significand_bits + step_count (collision-free on tested grid)",
